...
async def async_setup(hass: HomeAssistant, config: dict):
    ...
    _LOGGER.debug("Setting up Salus SP600 integration")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    _LOGGER.debug("Setting up Salus SP600 entry with port: %s", entry.data.get("zigbee_port"))
    ...
    _LOGGER.info("Salus SP600 setup complete for port: %s", entry.data.get("zigbee_port"))
    return True
